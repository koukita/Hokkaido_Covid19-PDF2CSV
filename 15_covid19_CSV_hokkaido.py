...
if(os.path.exists(CSV_path + "\\hokkaido_z" + dt_mmdd + ".csv")): #ファイルが存在するか確認 参考【https://techacademy.jp/magazine/18994】
    ...
    for i in range(len(csv_read_df)-1):
        if df_FLG and str(csv_read_df.iloc[i,1])!="nan": #フラグが立っている間の処理
            if str(csv_read_df.iloc[i,1])!="例目": #「例目」以外なら処理
                p_num = str(csv_read_df.iloc[i,1])  #例目
                p_residence = str(csv_read_df.iloc[i,2])  #居住地
                if " " in p_num: #例目に半角スペースが有る場合は、居住地とダブっていると判断
                    num_arr = p_num.split(" ") #スペースで区切り配列を作る
                    p_num = num_arr[0] #配列0が「例目」
                    p_residence= num_arr[1] #配列1が「居住地」
                ...
                p_sex = str(csv_read_df.iloc[i,no_kokuseki+1])  #性別
                p_age = str(csv_read_df.iloc[i,no_kokuseki+2])  #年齢
                
                if "未満" in p_age : #10歳未満か判別
                    p_age = "10歳未満"
                else:
                    p_age = p_age.replace("歳","")
                p_job = str(csv_read_df.iloc[i,no_kokuseki+3])  #職業
                p_status = str(csv_read_df.iloc[i,no_kokuseki+4])  #現状
                c_hassho = str(csv_read_df.iloc[i,no_kokuseki+5])  #発症日
                ...
                if "再陽性" in str(csv_read_df.iloc[i+1,no_kokuseki+8]):
                    #次の行に再陽性と記載されている場合
                    p_saiyousei = "1"
                else:
                    p_saiyousei = "0"
        ...
        if str(csv_read_df.iloc[i,0]) == "No":
            ...
            df_FLG = True
            for j in range(10):
                if str(csv_read_df.iloc[i,j]) == "国籍":
                    no_kokuseki = j
                    break 
