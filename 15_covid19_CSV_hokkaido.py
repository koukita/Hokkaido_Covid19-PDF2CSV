...
print("＝＝＝＝15_covid19_CSV_hokkaido.py＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝")
#今日の日付
...
if(os.path.exists(CSV_path + "\\hokkaido_" + dt_mmdd + ".csv")): #ファイルが存在するか確認 参考【https://techacademy.jp/magazine/18994】
    ...
    print("\\hokkaido_" + dt_mmdd + ".csv を処理中")
    ...
    for i in range(len(csv_old_df)-1):
        ...
        if "内道外" in str(csv_old_df.iloc[i,0]):
            break
        if "合計" in str(csv_old_df.iloc[i,0]):
            break

    day_total_shinkoukyoku = d_sorachi+d_ishikari+d_shiribeshi+d_iburi+d_hidaka+d_oshima+d_hiyama+d_kamikawa+d_rumoi+d_souya+d_ohotuku+d_tokachi+d_kushiro+d_nemuro+d_sonota
    ...
    tmp_se = pd.Series(["集計","北海道", d_sorachi,d_ishikari,d_shiribeshi,d_iburi,d_hidaka,d_oshima,d_hiyama,d_kamikawa,d_rumoi,d_souya,d_ohotuku,d_tokachi,d_kushiro,d_nemuro,d_sonota,0,0,day_total ], index=csv_shinkoukyoku_df.columns)
    ...
    tmp_se = pd.Series(["集計","北海道", d_age01,d_age10,d_age20,d_age30,d_age40,d_age50,d_age60,d_age70,d_age80,d_age90,0,0,day_total ], index=csv_age_df.columns)
    ...
    tmp_se = pd.Series(["集計","北海道", d_mushoujyou,d_keishou,d_tyoutoushou,d_jyoushou,0,0,day_total ], index=csv_status_df.columns)
    ...
    tmp_se = pd.Series(["集計","北海道", d_man,d_woman,0,0,day_total ], index=csv_sex_df.columns)
    ...
    for i in range(len(csv_old_df)-1):
        ...
        if  len(ruikei_arr) == 0:
            ...
            if "軽症・中等症" in str(csv_old_df.iloc[i,0]):
                print("累計検査数と陽性累計と現在患者数と軽症・中等症が一つになっている場合")
            ...
            if "軽症・中等症" in str(csv_old_df.iloc[i,1]):
                print("累計検査数と陽性累計と現在患者数が一つになっている場合")
            ...
            if "軽症・中等症" in str(csv_old_df.iloc[i,2]):
                print("累計検査数と陽性累計が一つになっている場合")
            ...
            if "軽症・中等症" in str(csv_old_df.iloc[i,3]):
                print("累計検査数と陽性累計が別になっている場合 ")
                retu = 0
        ...
        if str(csv_old_df.iloc[i,0]) == "の新規患者数":
            if str(csv_old_df.iloc[i+1,1]) == "調査中":
                #調査中の場合は便宜上「0」を入力
                ruikei_arr.append("0")  #濃厚接触者等の新規患者数
                ruikei_arr.append("0") #濃厚接触者等以外の新規患者数
            else:
                kensa_arr=str(csv_old_df.iloc[i+1,0]).split(" ")
        ...
        #全道レベル　2021-12-08追加
        if "全道 " in str(csv_old_df.iloc[i,0]) and "[" in str(csv_old_df.iloc[i,0]):
            d_stage = str(csv_old_df.iloc[i,0])
            d_stage = d_stage[3:4]  #全道レベルを抽出 4文字目を取り出す
            ruikei_arr.append("0")
            print("全道レベル=" + d_stage)
            ruikei_arr.append(d_stage) #ステージ

        #札幌市レベル　2021-12-08追加
        if "札幌市" in str(csv_old_df.iloc[i,0]) and "計" in str(csv_old_df.iloc[i,0]):
            d_stage = str(csv_old_df.iloc[i,0])
            d_stage = d_stage[4:5]  #札幌市レベルを抽出 5文字目を取り出す
            print("札幌市レベル=" + d_stage)
            ruikei_arr.append(d_stage) #ステージ

        #札幌市以外レベル　2021-12-08追加
        if "(札幌市" in str(csv_old_df.iloc[i,0]) and "] [" in str(csv_old_df.iloc[i,0]):
            d_stage = str(csv_old_df.iloc[i,0])
            d_stage = d_stage[5:6]  #札幌市レベルを抽出 5文字目を取り出す
            print("札幌市以外レベル=" + d_stage)
            ruikei_arr.append(d_stage) #ステージ
    ...
    if len(ruikei_arr) == 10:
        ...
        inputdata = simpleDialog.askstring("Input Box","全道のレベルの数値を入力してください",)
        if inputdata == None:
            ruikei_arr.append("0") #ステージ
        else:
            ruikei_arr.append(inputdata) #ステージ
        inputdata = simpleDialog.askstring("Input Box","札幌市のレベルの数値を入力してください",)
        if inputdata == None:
            ruikei_arr.append("0") #ステージ
        else:
            ruikei_arr.append(inputdata) #ステージ
        inputdata = simpleDialog.askstring("Input Box","札幌市以外のレベルの数値を入力してください",)
        if inputdata == None:
            ruikei_arr.append("0") #ステージ
        else:
            ruikei_arr.append(inputdata) #ステージ
    ...
    csv2_df = pd.DataFrame( columns=["累計検査数","陽性累計","現在患者数","軽症中等症","重症","死亡者累計","陰性累計","検査数","濃厚接触者数","濃厚以外数","ステージ","レベル全道","レベル札幌","レベル札幌以外"])
    tmp_se2 = pd.Series([ ruikei_arr[0], ruikei_arr[1], ruikei_arr[2], ruikei_arr[3], ruikei_arr[4], ruikei_arr[5], ruikei_arr[6], ruikei_arr[7], ruikei_arr[8], ruikei_arr[9], ruikei_arr[10], ruikei_arr[11], ruikei_arr[12], ruikei_arr[13]], index=csv2_df.columns)
    csv2_df = csv2_df.append(tmp_se2, ignore_index = True)
    csv2_df.to_csv(CSV_path + "\\day_total_ruikei_" + dt_mmdd + ".csv", index=None, encoding="CP932")
