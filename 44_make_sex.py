...
#ファイルが存在するか確認
if(os.path.exists(covid19_path + "\\hokkaido_04sex_day.csv")) and (os.path.exists(download_path + "\\day_total_sex_" + dt_mmdd_today + ".csv")):
    #pandasでCSVファイルを読み込み
    ...
    #ファイルをバックアップする
    shutil.copyfile(covid19_path + "\\hokkaido_04sex_day.csv", covid19_path + "\\backup\\hokkaido_04sex_day_backup" + dt_mmdd_today + ".csv")
    ...
    if today_day == kei_day: #最新の日付が今日の日付か確認
        ...
        df_covid19.to_csv(covid19_path + "\\自動作成ファイル\\hokkaido_04sex_day.csv",index=None,header=True, encoding="CP932")
else:
    print("【性別ごと】必要なファイルがない")
