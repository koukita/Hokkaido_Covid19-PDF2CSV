...
#ファイルが存在するか確認
if(os.path.exists(covid19_path + "\\hokkaido_01shinkoukyoku_day.csv")) and (os.path.exists(download_path + "\\day_total_shinkoukyoku_" + dt_mmdd_today + ".csv")):
    #pandasでCSVファイルを読み込み
    df_covid19 = pd.read_csv(covid19_path + "\\hokkaido_01shinkoukyoku_day.csv", encoding="CP932")
    df_total = pd.read_csv(download_path + "\\day_total_shinkoukyoku_" + dt_mmdd_today + ".csv", encoding="CP932")
    df_hokkaido = pd.read_csv(download_path + "\\day_hokkaido_shinkoukyoku_" + dt_mmdd_today + ".csv", encoding="CP932")
    ...
    #ファイルをバックアップする
    shutil.copyfile(covid19_path + "\\hokkaido_01shinkoukyoku_day.csv", covid19_path + "\\backup\\hokkaido_01shinkoukyoku_day_backup" + dt_mmdd_today + ".csv")
    ...
    if today_day == kei_day: #最新の日付が今日の日付か確認
        ...
        print("振興局ごとの日計の合計＝" + str(df_covid19["日計"].sum()))
        df_covid19.to_csv(covid19_path + "\\自動作成ファイル\\hokkaido_01shinkoukyoku_day.csv",index=None,header=True, encoding="CP932")
else:
    print("【振興局ごと】必要なファイルがない")
